...
const HEADER_HEIGHT = "h-[5.184rem] sm:h-[5.728rem]";
    ...
    <>
      ...
      <div
        ...
        className={`pointer-events-none fixed inset-x-0 top-0 z-40 ${HEADER_HEIGHT} bg-black transition-opacity duration-500 ${
          scrolled ? "opacity-[0.09]" : "opacity-0"
        }`}
      />
      ...
      <div
        aria-hidden="true"
        className={`pointer-events-none fixed inset-x-0 top-[5.184rem] z-40 h-10 bg-header-scrim transition-opacity duration-500 sm:top-[5.728rem] ${
          scrolled ? "opacity-[0.09]" : "opacity-0"
        }`}
      />
      <header className="fixed inset-x-0 top-0 z-50 flex items-center justify-between px-6 py-6 sm:px-10 lg:px-16">
        <a href="#hero" aria-label="Tarsius home">
          <Image src="/hero/tarsius-logo.svg" alt="Tarsius" width={1612} height={1054} className="h-[2.184rem] w-auto sm:h-[2.728rem]" priority />
        </a>
